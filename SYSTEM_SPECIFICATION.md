...
본 시스템은 **한국예탁결제원(KSD) 채권정보서비스** 및 **금융위원회 채권발행정보 V2 오픈 API**를 결합 연동하여, 국내 주요 채권(시중은행채, 카드채, 캐피탈채, 국고채 등)의 **2026년 상반기 발행/만기 실데이터 및 2026년 하반기(7~12월) 만기도래 예정액 100% 실데이터**를 자동으로 수집·분류·시각화하고, **규칙 기반 AI 시황 분석 보고서**를 생성하는 데이터 파이프라인 시스템입니다.
...
- **Automation / AI**: Antigravity 규칙 기반 수급 분석 엔진 (`analyst.py`)
...
- **🤖 [AI 리포트 탭]**: 2026년 수급 동향 실시간 경보 리포트
