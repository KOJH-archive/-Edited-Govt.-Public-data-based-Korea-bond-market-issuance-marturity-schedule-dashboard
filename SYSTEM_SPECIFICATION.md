...
### 🚨 Issue 6: 2026년 월별/섹터별 만기도래액 차트의 직관적 수치 확인용 수치 표 추가
- **요구사항 & 해결 방안**:
  - 기존 막대 그래프 시각화와 더불어 개별 월별/섹터별 정확한 금액 및 총액을 즉시 확인할 수 있도록 기능 요청.
  - 막대 그래프 직하단에 '월별 합계' 및 '총계'가 포함된 수치 피벗 테이블(`mat_pivot_display`)을 추가하여 그래프 시각화와 정확한 수치 데이터를 동시에 명확히 확인할 수 있도록 개선.
...
- **TAB 1 (2026년 월별 만기 현황)**: 월별/섹터별 스택 바 차트, 차트 하단 수치 표(월별 합계 및 총계 포함), 파이 차트, 상세 피벗 테이블 및 개별 종목 데이터 조회
